...
/* LOCAL-ONLY PATCH - do not upstream: expose db_enable unconditionally
 * (not just under #ifdef DEBUG) so it can be managed the same way as
 * ctgp_offset - see ONBOARDING.md GPU power limit investigation. With
 * Dynamic Boost left enabled (the driver's own default at probe time,
 * see init_db_and_ctgp() below), the GPU can exceed the ctgp_offset
 * target by using its full TPP headroom regardless of ctgp_offset's
 * value - confirmed empirically: ctgp_offset=0 (150W target) measured
 * ~175W sustained under real load with db_enable=1, and ~150W (matching
 * the target) with db_enable=0. TCC needs to manage this bit to make its
 * GPU power limit setting a real, enforced ceiling. */
