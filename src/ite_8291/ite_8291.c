...
static void color_scaling(struct hid_device *hdev, u8 *red, u8 *green, u8 *blue, bool row_col_set, u8 row, u8 col)
{
	 ...
	 * (0x600b) as TUXEDO's STELLARIS16I07/X6AR55xU quirk above. Reusing
	 * that curve since X6AR5xxY and X6AR55xU are TUXEDO's own board
	 * codes for closely related Stellaris 16 Gen7 boards. Empirically
	 * confirmed 2026-09-01: manually set all per-key LEDs to nominal
	 * white (255,255,255) at full brightness via sysfs
	 * (multi_intensity/brightness), owner visually confirmed a clean,
	 * bright white with no tint - correction curve is accurate for this
	 * unit's actual physical keyboard panel. */
	} else if (dmi_match(DMI_SYS_VENDOR, "SchenkerTechnologiesGmbH") &&
